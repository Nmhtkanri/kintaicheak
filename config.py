...
class Config:
    ...
    # 列: 列id, 列名, 給与体系, 社員番号条件, source_key, 体系別名条件, 備考
    # ⚠ 2026-08-28 に列を削って番号が総ずれしたため、キーを「列番号」から「列id」へ変えた。
    #   旧スキーマのファイルを読むと値が別の列へ入るので、パスごと _v2 に分けてある
    #   （旧ファイルはそのまま残り、差し替え前の exe は最後まで動く）。
    SHAROUSHI_COLUMN_MAPPING_CSV = os.environ.get(
        "SHAROUSHI_COLUMN_MAPPING_CSV", r"Z:\API連携\docs\社労士モード_列マッピング_v2.csv")
    ...
    # ------------------------------------------------------------------
    # 等級表（手作りの比較ブック。等級レンジ＋関東IT健保・協会けんぽ東京の料率）。
    # 同じフォルダに Codex が毎月置く公式資料（標準報酬月額_YYYY_MM.xlsx）があれば、
    # 対象年月に合うものを shaho_master.select_grade_table が優先して読む。そのときも
    # 関東IT健保の健康・介護の料率（公式資料に無い）はこのブックの「設定・出典」から取る。
    # 翌年度は「設定・出典」の料率と年度表記を更新する。年度が合わないと ShahoMasterError で停止する。
