...
def mini_template_csv(tmp_path):
    """会話で出てきた本物の雛形をベースにした最小 CSV"""
    path = tmp_path / "templates.csv"
    rows = [
        # No 16: 日勤 9:00-17:30
        {"No": "16", "勤怠スケジュール名称": "日勤", "略称(3文字以内)": "日勤",
         "勤怠スケジュール名ID": "1", "予定区分": "9998", "休日ID": "1",
         "出勤時刻(0:00~47:59)": "9:00:00", "退勤時刻(0:00~47:59)": "17:30:00",
         "休憩開始時間1(0:00~47:59)": "12:00:00", "復帰時間1(0:00~47:59)": "13:00:00"},
        # No 20: 遅番1 13:00-21:30
        {"No": "20", "勤怠スケジュール名称": "遅番1", "略称(3文字以内)": "遅1",
         "勤怠スケジュール名ID": "5", "予定区分": "9998", "休日ID": "1",
         "出勤時刻(0:00~47:59)": "13:00:00", "退勤時刻(0:00~47:59)": "21:30:00",
         "休憩開始時間1(0:00~47:59)": "17:00:00", "復帰時間1(0:00~47:59)": "18:00:00"},
        # No 21: D1 10:00-18:30
        {"No": "21", "勤怠スケジュール名称": "D1", "略称(3文字以内)": "D1",
         "勤怠スケジュール名ID": "6", "予定区分": "9998", "休日ID": "1",
         "出勤時刻(0:00~47:59)": "10:00:00", "退勤時刻(0:00~47:59)": "18:30:00",
...
def test_generate_new_templates(mini_template_csv, tmp_path):
    ...
    # 時刻が正しく入っている
    assert rows[0]["出勤時刻(0:00~47:59)"] == "12:30:00"
    assert rows[0]["退勤時刻(0:00~47:59)"] == "21:00:00"
    assert rows[1]["出勤時刻(0:00~47:59)"] == "16:30:00"
    assert rows[1]["退勤時刻(0:00~47:59)"] == "25:00:00"

    # 名称・ID も入っている
    assert rows[0]["勤怠スケジュール名称"] == "B勤"
