...
#      ※ 2026-08-13 の jinjer 設定変更で時給制の3項目が 1つずつ後ろへずれた
#        （前月超過勤務 2→3／前月深夜残業分 3→4／前月休日出勤時間分 4→5）。
#        判定は体系別名ベースなので仕訳の金額は変わらない。時給制の allowance2 は空になる。
...
#   体系別名を必ず「前月〜」のまま持っていくこと。汎用名「給与支給項目N」に戻ると
#   is_skip_item を抜け、差額調整(allowance24)と二重計上になる（下の SAGAKU_KEY 参照）。
...
#   ⚠ この再掲判定は「差額調整＝精算分の合計 or 単独一致」でしか効かない。時給制の差額調整は
#     総労働時間×時給 も含むので一致せず、ここでは捕まらない。時給制の前月〜3項目
#     （2026-08-13 以降は allowance3/4/5）は is_skip_item 側で落とすのが唯一の防波堤。
