...
# 月末の日跨ぎ勤務は「スケジュールの勤怠を暫定で働いたとみなし当月勤怠として計上する」
# 運用ルール（2026-07 谷津さん確認。jinjerは月をまたぐ範囲の汎用データを出せないため、
# 翌月に前月末日の行を突合・書き戻しすることはできない）。
TOKKI_MONTH_END_OVERNIGHT = "月末日跨ぎ勤務(請求側の翌月分は取得不可。スケジュール勤怠を暫定の正として当月計上)"
TOKKI_PREV_MONTH_TAIL_MATCHED = "前月末夜勤の後半(前月末日で突合)"
TOKKI_PREV_MONTH_TAIL_UNMATCHED = (
    "前月末夜勤の後半(前月分でスケジュール勤怠により暫定計上済み・自動書戻し不可)"
)
TOKKI_JINJER_SPLIT = (
