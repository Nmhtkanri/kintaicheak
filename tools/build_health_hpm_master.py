...
INSTITUTIONS = [
    ...
    ("ヘルスケアクリニック厚木", "1412910586", "", "TRUE",
     "2026-05受診（岡田さん）で使用"),
    ("関東ITソフトウェア健康組合(大久保健診センター)", "13X5035440", "", "TRUE",
     "健保内健診（関東ITS）の受診分はこのコードで登録する運用（2026-09-09 谷津さん確認）。"
     "2026-05受診（山長さん）で使用。コードに英字Xを含むので文字列で扱う。"
     "従来は桜十字グランフロント大阪クリニックの名で登録していたが、hpm.txt ではこのコードは大久保健診センター。"
     "桜十字は選択肢シート側の 2714111560 で出る"),
...
ALIASES = [
    ...
    ("健保内健診", "関東ITソフトウェア健康組合(大久保健診センター)"),
...
COURSES = [
    ...
    ("関東ITソフトウェア健康組合(大久保健診センター)", "人間ドックB（胃X線）", "12"),
...
ITEM_MAP = [
    ...
    # 眼圧は同友会の人間ドック様式にある（定期健診の様式には無い）。
    # 2026-07-21〜24 受診分で実在を確認し追加（HPM列 35=左 / 36=右）。
    ("眼圧", "眼圧（左）", 1, 35, "数値", "", ""),
    ("眼圧", "眼圧（右）", 1, 36, "数値", "", ""),
    ...
    # --- 所見（文章）と臓器別判定（2026-09-09 追加） ---
    # 値種別「所見」は原票の文章をそのまま所見列へ、「判定」は同じ項目の原票判定A〜Gを臓器別の
    # 判定列へ出す。まとめ判定列 183〜197（聴力判定=184 など）は従来どおり書かない。
    # 8列目「値変換」（任意）は原票の値→HPMの値。聴力は原票の「所見なし」をそのまま転記する
    # （2026-09-09 谷津さん決定）。HPMの語彙「正常」に寄せたくなったら 所見なし=正常 と書く。
    ("聴力", "聴力1000（左）", 1, 40, "所見", "", "原票の1KHz左。「所見なし」のまま転記"),
    ("聴力", "聴力1000（右）", 1, 41, "所見", "", "原票の1KHz右。「所見なし」のまま転記"),
    ("聴力", "聴力4000（左）", 1, 42, "所見", "", "原票の4KHz左。「所見なし」のまま転記"),
    ("聴力", "聴力4000（右）", 1, 43, "所見", "", "原票の4KHz右。「所見なし」のまま転記"),
    ("診察", "診察", 1, 204, "所見", "", ""),
    ("診察", "診察", 1, 203, "判定", "", ""),
    ("心電図", "安静時心電図", 1, 206, "所見", "", "原票の心電図（安）"),
    ("心電図", "安静時心電図", 1, 205, "判定", "", ""),
    ("胸部X線", "胸部X線", 1, 210, "所見", "", "原票の胸部X線（呼）"),
    ("胸部X線", "胸部X線", 1, 209, "判定", "", ""),
    ("胸部X線", "心血管系", 1, 212, "所見", "", "原票の胸部X線（心）"),
    ("胸部X線", "心血管系", 1, 211, "判定", "", ""),
    ("眼底", "眼底", 1, 238, "所見", "", ""),
    ("眼底", "眼底", 1, 237, "判定", "", ""),
    ("胃部", "胃部", 1, 216, "所見", "X線", "健診種別がバリウム（12）のとき"),
    ("胃部", "胃部", 1, 215, "判定", "X線", ""),
    ("胃部", "胃部", 1, 218, "所見", "内視鏡", "健診種別が胃カメラ（13）のとき"),
    ("胃部", "胃部", 1, 217, "判定", "内視鏡", ""),
    ("腹部超音波", "肝臓", 1, 220, "所見", "", ""),
    ("腹部超音波", "肝臓", 1, 219, "判定", "", ""),
    ("腹部超音波", "胆嚢", 1, 222, "所見", "", ""),
...
def build(header_columns: list[str]) -> Workbook:
    ...
    _sheet(
        ...
        ["分類", "項目名", "測定回", "HPM列番号", "値種別", "検査方式", "備考", "値変換"],
        [tuple(r) + ("",) * (8 - len(r)) for r in ITEM_MAP],
        widths=[14, 24, 8, 12, 10, 12, 52, 30],
    )
