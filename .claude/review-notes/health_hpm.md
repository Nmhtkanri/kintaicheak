...
## 既知の落とし穴
- 13X5035440 の機関名は 2026-09-09 にマスタ Excel を「関東ITソフトウェア健康組合(大久保健診センター)」へ改名し、2026-09-10 に `tools/build_health_hpm_master.py` の初期データも追随させた（`--force` 再生成で旧名に戻る問題は解消）。桜十字グランフロント大阪は選択肢シート側の 2714111560 で出る。
