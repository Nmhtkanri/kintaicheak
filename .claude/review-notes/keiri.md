...
## 業務ルール
- 社保の 1 か月ズレ：会社負担＝当月分／本人控除＝前月分（`keiri_engine.py` の `build_shaho` 周辺）。
...
## 宿題
- **子育て支援金 506 円が未対応（2026-09-10 時点も未対応を確認）。** 健保の主取引はマスタの kenpo 構成（other15／other16／child_support）を前月分 `st_prev` に当てて合算しているが、child_support は本人控除＝前月分なので前々月分を拾う形になる。補正コードも免除判定（deduction29/31 のみ）も child_support を見ていない。直すと全員に影響するので、実データで回して差分を見る作業が要る。
