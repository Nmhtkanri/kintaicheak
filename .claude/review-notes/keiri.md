...
## 宿題
- **月変月の健保①が最終 CSV と合わない（2026-09-10 判明、未対応・要判断）。** 2026-08 実行分（7 月分計上）を最終 CSV と突合すると、4 月昇給→7 月月変の 33 人で生成が数千円ずつ小さい。原因は jinjer の 7 月明細の会社負担（other15/16）が旧標準報酬のまま（月変が 7 月給与計算の後に登録された）で、経理担当は 8 月明細の本人控除（7 月分・新額）を採用していたこと。①の読み元を「前月明細の会社負担」から「当月明細の本人控除」に変えるかは業務判断。月変の無い月は両者同額（2026-05→06 で 219 人中 210 人一致、残り 9 人は退職 2 倍・育休免除の既知ケース）。2024027（遠田さん）は最終 CSV の 12,978＋322＝13,300 が正で、手元の 8 月明細キャッシュ（8/14 取得）が古かっただけ（jinjer は 8/17 15:13 に更新済み。API で取り直して確認）。2025001（森田さん、8/31 退職）は名簿キャッシュに退職日が無かったための見かけの差で、退職日入りの名簿で作り直すと 19,210 円 × 2 行で完全一致した。**オフラインで突合するときは、名簿キャッシュ（raw/roster.json）と給与明細キャッシュ（raw/salary_statements_*.json の fetched_at）が最新かを先に確認する。給与確定後も jinjer 側で個別に直されることがあり（2026-08 は 8/13 確定→8/17 修正）、古いキャッシュで突合すると存在しない差が出る。**
