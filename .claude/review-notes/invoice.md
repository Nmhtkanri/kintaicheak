...
- 提出用 PDF（請求書 Excel＋勤怠 PDF。2026-08-20 実装。exe 再ビルドに pypdf が必要）。**対象者は設定 CSV `請求書モード_PDF作成設定.csv` が正で、コードに人数の直書きは無い。** 2026-09-10 時点の CSV は 2 社 3 名（アクシス＝細川さん／NIC ソフト＝熊崎さん・岡崎さん）。実装時は 2 社 4 名だったが、アクシスの大村さんは退職のため意図的に外した（2026-09-10 谷津さん確認）。
